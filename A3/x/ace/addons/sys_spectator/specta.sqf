...
if (!isNil QUOTE(GVAR(exit_spectator))) then {waitUntil {isNil QUOTE(GVAR(exit_spectator))}};
...
if (isNil QUOTE(GVAR(no_one_alive))) then {GVAR(no_one_alive) = -1};
...
GVAR(specstrings) = [(localize "STR_ACE_SPECT_DEAD") + " ","ace_sys_spect_Marker%1",localize "STR_ACE_SPECT_UNKNOWN",localize "STR_ACE_SPECT_PILOT",localize "STR_ACE_SPECT_DRIVER",localize "STR_ACE_SPECT_GUNNER",localize "STR_ACE_SPECT_COMMANDER",localize "STR_ACE_SPECT_SELCAM",localize "STR_ACE_LIFE_TIME_REV"];
...
GVAR(dirmarker) = createMarkerLocal [QUOTE(GVAR(sdir_marker)), [0, 0, 0]];
