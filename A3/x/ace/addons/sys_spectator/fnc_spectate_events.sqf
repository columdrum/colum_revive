...
switch (_type) do {
	...
	case "MapClick": {
		...
		_idxx = 0;
		{
			if ((side _x) in GVAR(ShownSides)) then {
				if (_x in GVAR(units)) then {
	...
	case "KeyUp": {
		...
		switch(_key) do {
			...
			case 2: {GVAR(cameraIdx) = 0};
			case 3: {GVAR(cameraIdx) = 1};
			case 4: {GVAR(cameraIdx) = 2};
			case 5: {GVAR(cameraIdx) = 3};
			case 6: {GVAR(cameraIdx) = 4};
	...
	case "UnitFired": {
		...
		if (ctrlVisible KCMAPFULL) then {
			...
			_m2 = createMarkerLocal ["hd_dot", getPosASL _o];
			...
			if (_type in ["shotMissile", "shotRocket", "shotShell", "shotTimeBomb", "shotPipeBomb", "shotMine", "shotSmoke"]) then {
				_opos = getPosASL _o;
				_m = createMarkerLocal ["hd_dot", [(_opos select 0) + (sin _dir) * _len, (_opos select 1) + (cos _dir) * _len, 0]];
			...
			// Missile camera;
			if (GVAR(UseMissileCam) == 1 && !GVAR(DroppedCamera)) then {
				if (_u == vehicle GVAR(target) && (_type in ["shotMissile", "shotRocket", "shotShell"]) && !GVAR(MissileCamActive)) then {
					_name = getText(configFile >> "CfgWeapons" >> _w >> "displayName");
