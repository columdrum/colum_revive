...
GVAR(dropAction) = player addAction [format[__DROP,(_name_dragee)], QUOTE(PATHTOF(drop_body.sqf)), [_dragee, 0], 0, false, true];
GVAR(carryAction) = player addAction [format[__CARRY,(_name_dragee)], QUOTE(PATHTOF(carrydead.sqf)), _dragee, 0, false, true];
		...
		if (!(_vec isKindOf "ParachuteBase") && !(_vec isKindOf "BIS_Steerable_Parachute") && !(_vec isKindOf "UAV")) then {
			...
			if (_cargocrew < _tps) then {
				if (GVAR(loadAction) == -3333) then {
					GVAR(loadAction) = player addAction [format[__LOAD,(_name_dragee), typeOf _vec], QUOTE(PATHTOF(load.sqf)), [_dragee, _vec], 0, false, true];
				};
			} else {
				if (GVAR(loadAction) != -3333) then {
