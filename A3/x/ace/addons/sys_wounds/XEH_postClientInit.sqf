...
// bleeding
[] spawn COMPILE_FILE(bleedFX);

// bleeding
[] spawn COMPILE_FILE(painFX);

// bandage, morphine action player (fsm?)
if (!isMultiplayer) then {
	[] spawn COMPILE_FILE(playeractions);
};
...
["UST mod", "Interaccion", {false}, [DIK_LWIN,false,false,false]] call cba_fnc_registerKeybind;
["UST mod", "Interaccion propia", {false}, [DIK_APPS,false,false,false]] call cba_fnc_registerKeybind;
	 
if (!isNil "ace_sys_wounds_enabled") then {


/*
["UST mod", "Interaccion propia", ["player", [], 4, [QPATHTO_F(fnc_menuDef_Self), "main"]], [DIK_APPS,false,false,false]] call CBA_fnc_registerKeybindToFleximenu;

["UST mod", "Interaccion", ["CAManBase", [], 5, [QPATHTO_F(fnc_menuDef), "main"]] , [DIK_LWIN,false,false,false]] call CBA_fnc_registerKeybindToFleximenu;
["UST mod", "Interaccion", [["Car", "Tank", "Air", "ACE_Stretcher"], [], -2, [QPATHTO_F(fnc_menuDef), "vehicle"]] , [DIK_LWIN,false,false,false]] call CBA_fnc_registerKeybindToFleximenu;

*/

//org

	 _interaction_key_get = ["UST mod", "Interaccion"] call cba_fnc_getKeybind;
	 _interaction_key_sef_get = ["UST mod", "Interaccion propia"] call cba_fnc_getKeybind;
	 
	if (_interaction_key_get >= 0) then {
		_keybind = (cba_keybinding_handlers select _interaction_key_get) select 2;
		 ace_sys_interaction_key=[_keybind select 0, [_keybind select 1, _keybind select 2,_keybind select 3]];
	};
	
	if (_interaction_key_sef_get >= 0) then {
		_keybind = (cba_keybinding_handlers select _interaction_key_sef_get) select 2;
		ace_sys_interaction_key_self=[_keybind select 0, [_keybind select 1, _keybind select 2,_keybind select 3]];
	};
	...
	[["Car", "Tank", "Air", "ACE_Stretcher"], [ace_sys_interaction_key], -2, [QPATHTO_F(fnc_menuDef), "vehicle"]] call CBA_ui_fnc_add;
	
	//movido, guear pss
	["CAManBase", [ace_sys_interaction_key], 4, ["\x\ace\addons\sys_interaction\fnc_menuDef.sqf", "main"]] call CBA_ui_fnc_add;
	...
	while {true} do {
		...
		if (_burn || _unitBurns) then {
			...
			if (_i > _bc && !_unitBurns) then {
				...
				["ace_sys_wounds_checkburn", [_unit]] call CBA_fnc_localEvent; // LocalEvent; Burning runs everywhere, so we only process local units
				_i = 0;
			} else {
				INC(_i);
			};
