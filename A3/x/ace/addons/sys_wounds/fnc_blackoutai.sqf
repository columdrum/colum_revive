...
if (animationState _unit in ["ladderriflestatic","laddercivilstatic","ladderrifleuploop","ladderciviluploop"]) then {
...
while {(_unit call FUNC(isUncon)) && alive _unit} do {
	...
	if (vehicle _unit == _unit && _unit call FUNC(isUncon) && isNull (_unit getVariable ["ace_w_carry", objNull])) then {
		if ((_unit call CBA_fnc_getUnitDeathAnim) != "DeadState") then {
			[_unit, 100] call FUNC(animator2);
		};
	};

	private ["_vecp","_endtime"];
	_vecp = (vehicle _unit != _unit);
	_endtime = time + (_uncontime * 2);
	waitUntil {time >= _endtime || !alive _unit || (_vecp && (vehicle _unit == _unit))};
	if (!alive _unit) exitWith {};
	if (vehicle _unit == _unit && _unit call FUNC(isUncon) && isNull (_unit getVariable ["ace_w_carry", objNull])) then {
		if ((_unit call CBA_fnc_getUnitDeathAnim) != "DeadState") then {
			[_unit, 100] call FUNC(animator2);
...
if (alive _unit) then {
	...
	sleep 5 + random 5;
	if (!(_unit call FUNC(isUncon)) && alive _unit) then {
