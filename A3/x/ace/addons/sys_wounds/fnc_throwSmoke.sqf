...
PARAMS_1(_unit);
private ["_cant","_smokemuz","_smokeshell","_smokeveh","_thism","_pos","_dir","_throwdist","_smoke","_anim"];

_cant = {
	private "_ret";
	_ret = _this call FUNC(isUncon) || !alive _this;
	if (isNumber (configFile >> "CfgMovesMaleSdr" >> "States" >> (animationState _this) >> "canPullTrigger")) then {
		_ret = _ret || getNumber (configFile >> "CfgMovesMaleSdr" >> "States" >> (animationState _this) >> "canPullTrigger") == 0;
	};
	_ret
};
...
} forEach (_unit nearTargets 1200);
