...
while {true} do {
	...
	while {alive player} do {
		waitUntil {(player getVariable "ace_w_pain") >= 0.1 || !alive player || (player call FUNC(isUncon))};
		...
		while {(player getVariable "ace_w_pain" >= 0.1) && alive player && (!(player call FUNC(isUncon)))} do {
			_st = time + 2.5;
			waitUntil {time > _st || !alive player || (player call FUNC(isUncon))};
			if (!alive player || (player call FUNC(isUncon))) exitWith {};
			...
			switch (true) do {
				...
				case (_pain > 0.2 && _pain <= 0.3): {
					GVAR(blur) ppEffectEnable true;
					GVAR(blur) ppEffectAdjust [0.4];
					GVAR(blur) ppEffectCommit 1;
					_sleep_time = 3;
				};
				case (_pain > 0.3 && _pain <= 0.5): {
					GVAR(blur) ppEffectEnable true;
					GVAR(blur) ppEffectAdjust [0.6];
					GVAR(blur) ppEffectCommit 1;
					_sleep_time = 4;
				};
				case (_pain > 0.5 && _pain <= 0.6): {
					GVAR(blur) ppEffectEnable true;
