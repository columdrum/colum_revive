...
if (isNil "_aceweh") then {
	if (isNil "ace_w_no_handledamage") then {
		_add=_woundsEnabled; // do nothing if not enabled, handledamage could overwrite any other script when wounds disabled
	/*
...
if (_woundsEnabled) then {
	...
	if ([_u] call FUNC(isMedic)) then {
		[_u] execFSM QUOTE(PATHTOF(AIAttendantHandling.fsm));
	};
};
