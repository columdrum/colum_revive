...
private ["_dragee", "_unit", "_nos", "_vec", "_cargocrew", "_vecrole", "_tps"];
...
if (isPlayer _dragee) then {
	...
	[QUOTE(ace_sys_interaction_confirmation), [_dragee, player, -1, _action]] call CBA_fnc_globalEvent;
	_endtime = time + 20;
} else {
...
sleep 14;
_dragee attachto [_unit,[0, -0.1, -1.25],"RightShoulder"];

[QGVAR(dir), _dragee] call CBA_fnc_globalEvent;

_found_anim = false;
_draganims = ["amovpercmrunsraswrfldr",
...
//[player, 80] call ace_sys_stamina_fnc_inc_mass; // 80 kg, default
...
GVAR(dropAction) = player addAction [format[__DROP, _name_dragee], QUOTE(PATHTOF(drop_body.sqf)), [_dragee, 0, true], 0, false, true];
