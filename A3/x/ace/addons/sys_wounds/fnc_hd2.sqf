...
if (_part == "") then {   // ** first triggering of the EH - no part
	...
	if (_ammo == "" && isNull _injurer) then { // helper for functions that call hd directly, for example in backblast, etc, injurer has to be null
		if (!_withpmr) then {
			...
			if (_bd < 1) then { [_unit, _state] call FUNC(NoAmmoCheck) };
			...
			_bd = _unit getVariable (GVAR(parts) select 1);
			switch (true) do {
				case (_bd > 0 && _bd < STATE_800_HEAD_BODY_DAM_MAX): {
				...
				case (_bd >= STATE_800_HEAD_BODY_DAM_MAX && _bd < STATE_801_HEAD_BODY_DAM_MAX): {
		...
		if (!_alv) then{
			...
			if (!_isbb && !([_vehicle] call ace_sys_vehicledamage_fnc_enabled)) then {
				...
				if (_partDamage > MIN_PART_DAM) then {
					...
					if (!ace_sys_spectator_SPECTATINGON) then {
						if (_isp && (_ammo isKindOf "bulletBase")) then {
	...
	if (!_isp && _rv > 0 && !_dead) then {
		if (!(_unit call FUNC(isUncon)) && !(_unit getVariable ["ace_w_check", false])) then {
	...
	switch (_part) do {	
		case "head": {_headBody = true; _coef = HEAD_DAM_COEF};
	...
	if (!isNil "_ha") then { if (count _ha > 0) then { EXPLODE_7(_ha,_vel,_mass,_cal,_penetrationMultiplier,_dir,_ke,_damage) } };
	...
	_penetrationCoefficient = BASE_PENETRATION_COEF * _penetrationMultiplier;
	if (_ke) then { // use energy to calculate damage
		TRACE_6("wounds debug 1", _ke, _tp, _vel, _damage,_partArmor,_bloss);
		switch (_tp) do {
			case 0: { // head
				if (_partarmor >= 0.85) then { // unit has helmet
					if (_dir > 70 && _dir < 290) then { // back
		...
		_penetration = ((_vel * _mass) / (_cal^1.7)) * _penetrationCoefficient;
		...
		_newPartDamage = switch (_part) do {
			case "head": { 1 min (_nd + _prevDamage) };
			case "body": { 1 min (_nd + _prevDamage) };
			case "hands": { _div = HANDS_BLOSS_DIV; _nd + _prevDamage };
			case "legs": { _nd + _prevDamage };
			default {diag_log["Wounds Critical error, part not found",_part,_this,_nd,[_bloodLossComponent , _energyComponent,_diff , _damage]];};
		};
		...
		// Knockdown
		if (_diff > 0.1 && !_inVehicle && !(_unit getVariable ["ace_w_falling", false]) && _part != "hands") then { [_unit, _energy, _dir, _force_fall] call FUNC(fall) };
		...
		if (_ammo == "") then {
			...
			if (_headBody && _injurer == _unit && !_inVehicle) then {
				...
				if (_nd > FALL_DAM_TRESHOLD) then {_nd = _nd / FALL_DAM_DIV} else { _nd = 0 };
	...
	if (typename _newPartDamage !=typename 0) then {diag_log["colum error",_ke,_nd,_newPartDamage,_bloss,_headbody];};
	...
	if (_headbody) then {
		...
		switch (true) do {
			case (_newPartDamage > 0 && _newPartDamage < STATE_800_HEAD_BODY_DAM_MAX): {
				[_unit, if (_penetrated) then { _bloss } else { 0 }, _bloss, 0] call FUNC(vchange);
				if (_state <= 800) then {
					_unit setVariable ["ace_w_state",800]; // state 800 = bleeding signs, pain
				};
			};
			case (_newPartDamage >= STATE_800_HEAD_BODY_DAM_MAX && _newPartDamage < STATE_801_HEAD_BODY_DAM_MAX): {
				_bloss = _bloss * STATE_801_BLOSS_COEF;
				[_unit, if (_penetrated) then { _bloss } else { 0 }, _bloss, 1] call FUNC(vchange);
				if (_state <= 801) then {
					_unit setVariable ["ace_w_nextuncon", time];
					_unit setVariable ["ace_w_state",801]; // state 801 = blackouts every now and then, bleeding signs, pain
				};
		...
		switch (true) do {
			case ((_newPartDamage > 0) && (_newPartDamage < STATE_800_ARMS_LEGS_DAM_MAX)): {
				[_unit, if (_penetrated) then { _bloss } else { 0 }, _bloss, 0] call FUNC(vchange);
				if (_state <= 800) then {
					_unit setVariable ["ace_w_state",800];
				};
			};
			case ((_newPartDamage >= STATE_800_ARMS_LEGS_DAM_MAX) && (_newPartDamage < STATE_801_ARMS_LEGS_DAM_MAX)): {
				_bloss = _bloss * STATE_801_BLOSS_COEF;
				[_unit, if (_penetrated) then { _bloss } else { 0 }, _bloss, 1] call FUNC(vchange);
				if (_state <= 801) then {
					_unit setVariable ["ace_w_nextuncon", time];
	...
	if (_penetrated && _ammo != "" && !_dead) then {
		...
		TRACE_1("wounds debug xtrabloss", _extraBl);
	};
	TRACE_3("wounds debug 2", _bloss,_newPartDamage,_state);
};

if (_startStateHandler && !_dead) then { // Start Statehandler if not already running, and not dead
	if (_unit getVariable ["ace_w_fsm", 0] == 0) then {
