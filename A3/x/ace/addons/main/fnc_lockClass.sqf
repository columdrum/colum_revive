...
_unit setVariable [QUOTE(GVAR(types)), _types];
_unit setVariable [QUOTE(GVAR(default_weapons)), _weapons];

if !(local _unit) exitWith {};

[_unit] spawn {
	PARAMS_1(_unit);
	_defaultWeapons = _unit getVariable QUOTE(GVAR(default_weapons));
	_defaultTypes = _unit getVariable QUOTE(GVAR(types));
