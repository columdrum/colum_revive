...
if (isNil QUOTE(GVAR(bulletcamera_timeout))) then { GVAR(bulletcamera_timeout) = 10 }; // default 10s timeout

if !(isNil QUOTE(GVAR(bulletcamera))) exitWith {};
...
waitUntil {
	player sideChat format ["Velocity: %1 m/s Distance: %2 m", round ((velocity _projectile) call BIS_fnc_magnitude),10*(round (((vehicle player) distance _projectile)/10))];
	if (_projectile isKindOf "MissileBase") then {
		GVAR(bulletcamera) camSetTarget _projectile;
		_velocity = velocity _projectile;
		_speed = _velocity call BIS_fnc_magnitude;
